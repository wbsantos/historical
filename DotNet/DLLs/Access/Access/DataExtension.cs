...
namespace Access
{
    public static class DataExtension
    {
        ...
        public static T ToEnum<T>(this DataRow row, string colName) where T : struct, IConvertible
        {
            return IsDBNull(row[colName]) ? default(T) : (T)row[colName];
        }
        public static long? ToInt64Nullable(this DataRow row, string colName)
        {
            return IsDBNull(row[colName]) ? null : (int?)Convert.ToInt64(row[colName]);
        }

        public static long ToInt64(this DataRow row, string colName)
        {
            return IsDBNull(row[colName]) ? 0 : Convert.ToInt64(row[colName]);
        }
